...
function twentytwelve_setup() {
	...
	set_post_thumbnail_size( 300, 200,true ); // Unlimited height, soft crop
}
...
function twentytwelve_scripts_styles() {
	...
	wp_enqueue_script('share42', get_template_directory_uri().'/share42/share42.js');
	
	wp_enqueue_script( 'ya_icons', '//yandex.st/share/share.js');
...
$loginoutlink = wp_loginout('index.php', false);
  ...
  if (is_user_logged_in() && $args->theme_location == 'header-menu') {
       $items .= '<li><a id="button-reg" href="/wp-admin/">My account</a></li>';
       $items .= '<li>'. $loginoutlink .'</li>';
    }
    elseif (!is_user_logged_in() && $args->theme_location == 'header-menu') {
    	$items .= '<li ><a id="button-reg" href="/wp-login.php?action=register">Registration</a></li>';
		$items .= '<li>'. $loginoutlink .'</li>';
    }
    return $items;
...
function top_authors($number = 10) {
    ...
    if ($blogusers) {
        ...
        foreach ($uc as $key => $value) {
            $i++;
            if ($i <= $number) {
            	$author_bio_avatar_size = apply_filters( 'twentytwelve_author_bio_avatar_size', 100 );
            	$avatar=get_avatar( $key, $author_bio_avatar_size );
                $user = get_userdata($key);
                $author_posts_url = get_author_posts_url($key);
                $post_count = $value;
                if ($post_count > 0) {
                    echo '<li> '. $avatar .  '<a href="' . $author_posts_url .'">' . $user->display_name . '</a><br><div><b>' . $post_count . '</b> completed orders</div></li>';
        ...
        foreach ($comments as $comment) {
            ...
            ?>
                <li>
                    ...
                    <div style="float: left;"><?php echo get_avatar($comment,$size='10' ); ?></div>
...
function recent_comments_remak ($number=5, $length=150) {
		...
		foreach($comments as $comment) :
			...
			echo '<li class="recentcomments">' . $comment->comment_author .' about "' . get_the_title($comment->comment_post_ID) . '"<br /><br /><a href="' . get_comment_link($comment->comment_ID) . '">' . substr($comment_content_short, 0, strrpos($comment_content_short, ' ')) . '...</a></li>';
		endforeach;
		echo '</ul>';
}
